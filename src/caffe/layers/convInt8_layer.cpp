...
inputInt8.Reshape(col_buffer_shape_);
    ...
    //for(int i=0;i<this->blobs_int8_[0].get()->count();i++) std::cout<<int(this->blobs_int8_[0].get()->mutable_cpu_data()[i])<<std::endl;


}

template <typename Dtype>
void ConvInt8Layer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void ConvInt8Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    LOG(FATAL)<<"NOT IMPLEMENTED";
}

template <typename Dtype>
void ConvInt8Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
