...
template <typename Dtype>
__global__ void im2col_gpu_quanti_kernel(const int n, const Dtype* data_im,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
    const int height_col, const int width_col,
    signed char* data_col, const Dtype t1, const Dtype t2, const Dtype unit_scale) {
  CUDA_KERNEL_LOOP(index, n) {
    ...
    signed char* data_col_ptr = data_col;
    ...
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        ...
        if(h_im >= 0 && w_im >= 0 && h_im < height && w_im < width)
        {
          ...
          {
            *data_col_ptr = (signed char)(round(data_im_ptr[pos]*unit_scale));
          }
        }
        else
        {
...
template <typename Dtype>
void im2col_gpu_quantized(const Dtype* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
...
template void im2col_gpu_quantized<float>(const float* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w,
    signed char* data_col, const float t1, const float t2, const float unit_scale);
template void im2col_gpu_quantized<double>(const double* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w,
    signed char* data_col, const double t1, const double t2, const double unit_scale);
